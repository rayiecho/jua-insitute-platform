...
export default async function DashboardPage() {
  ...
  const firstName = learner?.first_name ?? 'there';

  return (
    <DashboardShell>
      <h1 className="font-serif text-3xl font-semibold text-ink">Hello, {firstName}</h1>
      ...
      <div className="mt-4 flex flex-col gap-3">
        {(enrollments ?? []).map((enrollment) => {
          ...
          return (
            ...
            >
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-gold/15 font-serif text-xl font-semibold text-gold-dark">
                {(course?.title ?? 'P').charAt(0)}
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="truncate font-medium text-ink">{course?.title ?? 'Program'}</h3>
              ...
              {continueNode && (
                <Link
                  ...
                  className="shrink-0 rounded bg-gold px-4 py-2 text-sm font-semibold text-ink"
                >
                  Continue
                </Link>
              )}
