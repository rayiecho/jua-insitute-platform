...
export default async function ProgramPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <SiteHeader />
      <main className="w-full max-w-3xl flex-1 px-4 py-8 sm:px-8 sm:py-10">
        <span className="text-xs font-semibold tracking-wide text-tan uppercase">{course.difficulty_level}</span>
        <h1 className="mt-2 font-serif text-3xl font-semibold text-ink sm:text-4xl">{course.title}</h1>
